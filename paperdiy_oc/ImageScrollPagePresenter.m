...
@implementation ImageScrollPagePresenter
...
- (void)reloadView
{
    ...
    //设参数
    NSDictionary *parameters = @{@"modelId":self.modelId};
    //发请求
...
- (void)drawView
{
    ...
    {
        ImageScrollPageViewCell *cell = [[ImageScrollPageViewCell alloc] initWithFrame:CGRectMake(imageW*i, imageY, imageW, imageH)];
    ...
    // 设置当前在第几页
    self.pageControl.currentPage = [self.models objectAtIndex:self.indexPath.row].vSort;
    // 设置srollView的代理
...
//    NSLog(@"@index -> %@",index);
    int page = [[self.models objectAtIndex:index].vSort intValue];
