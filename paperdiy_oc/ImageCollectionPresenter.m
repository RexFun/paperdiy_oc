...
@implementation ImageCollectionPresenter
...
- (void)pullDownRefresh
{
    ...
    //设参数
    NSDictionary *parameters = @{@"modelId":self.modelId, @"offset":@"0", @"limit":@"5"};
    //发请求
...
- (void)pullUpRefresh
{
    ...
    manager.responseSerializer.acceptableContentTypes = [NSSet setWithObjects:@"application/json", @"text/json", @"text/javascript",@"text/html", @"text/plain",nil];
    //设参数
    NSDictionary *parameters = @{@"modelId":self.modelId, @"offset":[NSNumber numberWithLong:self.models.count], @"limit":@"5"};
    //发请求
    [manager POST:[AppUtil getActionUrlInPlistWithKey:@"ImagePageAction"] parameters:parameters
    success:^(NSURLSessionTask *task, id responseObject)
    {
